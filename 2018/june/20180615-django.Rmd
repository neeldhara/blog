...
In preparing for a session in a Python workshop, I finally got around to exploring the process of building a webapp powered by Django. I picked up the excellent [Hello Web App](https://hellowebbooks.com/learn-django/)^[I'd happily recommend this book for anyone with a little HTML/CSS exposure and some familiarity with basic Python concepts.] book by [Tracy Osborn](https://limedaring.com/) for my preparation. In this post, I am going to document the process - just in case I have to do this again ☕️

The tutorial in *Hello Web App* goes over the process of building a generic "collection of things" with Django^[Incidentally, another nice tutorial that follows this approach is [Practical JavaScript](https://watchandcode.com/p/practical-javascript)]. In this exposition, I'll focus on a more concrete example, because it was more fun for me to build a tangible concept that I might actually use later.

So, I am going to build a collection of talk announcements at IIT Gandhinagar, which I intend to be displayed, on the frontend, as a slideshow. I figure that this website can then be loaded up on any internet-connected display and serve as a virtual notice board. For the frontend design, I'm going to be stealing the [Webslides](https://webslides.tv/) framework, which is a really clean HTML/CSS template for making slideshows powered by Javascript.
...
If you are not sure how to get to this point, I'd suggest looking up an installation/setup guide such as [this one](https://docs.djangoproject.com/en/2.0/intro/tutorial01/) from the official documentation or [this one](https://tutorial.djangogirls.org/en/django_start_project/) from Django Girls.
...
The business with the `TalkAdmin` class is optional, but it lets you do nice things like prepopulate fields where relevan: for instance, if we had a slug field, we could have said something like:
...
![](../hellodjango-screenshot-05.png)

Now let's go ahead and add some sample data in here:

![](../hellodjango-screenshot-06.png)
...
<div class='cloudapp-embed' data-slug='3K3z3c3v3J1V'><a href='https://cl.ly/3K3z3c3v3J1V'>Screen Recording 2018-06-16 at 01.34 PM.gif</a><script async src='https://embed.cl.ly/embedded.gz.js' charset='utf-8'></script></div>
...
<div class='cloudapp-embed' data-slug='2h3W2t2m340w'><a href='https://cl.ly/2h3W2t2m340w'>Screen Recording 2018-06-16 at 01.42 PM.gif</a><script async src='https://embed.cl.ly/embedded.gz.js' charset='utf-8'></script></div>
...
1. Registration (possibly even single-sign ons)
2. A Login Flow (optionally, also an onboarding flow)
3. Reset Password
4. The ability for users to view user-specific data
5. The ability for users to add/edit/delete user-specific data
